...
const getChirpyRedStaus = `-- name: GetChirpyRedStaus :one
SELECT is_chirpy_red
FROM users
WHERE id = $1
`

func (q *Queries) GetChirpyRedStaus(ctx context.Context, id uuid.UUID) (bool, error) {
	row := q.db.QueryRowContext(ctx, getChirpyRedStaus, id)
